 ...
 * one view.
 *
 * RecyclerView Adapters must implement both the android.support.v7.widget.RecyclerView.Adapter#onCreateViewHolder(android.view
 * .ViewGroup, int)
 * and {@link RecyclerView.Adapter#onBindViewHolder(RecyclerView.ViewHolder,
 * int)} methods, as opposed to just the {@link android.widget.BaseAdapter#getView(int,
 * View,
 * ViewGroup)} method in a ListView Adapter.
 *
 * Because the  android.support.v7.widget.RecyclerView.Adapter#onCreateViewHolder(android.view
 * .ViewGroup, int) method only provides us the ViewGroup and a View Type, we must keep a mapping
 * of
 * (unique) view types in this merge adapter to each sub adapter, so we know which adapters'
 * onCreateViewHolder method to call.
 *
 * The {@link LocalAdapter} class
 * is used to maintain this mapping, and the  mViewTypeIndex field is used to provide a unique
 * view type.
 *
 *
 * -------------
 ...
 * -------------
 *
 * There currently exists a bug when merging Views and Adapters. The RecyclerView throws the
 * following error:
 *
...
public class MergeRecyclerAdapter<T extends RecyclerView.Adapter> extends RecyclerView
     ...
     * Adapter by iterating through Adapters and
    ...
    @Override public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        for (LocalAdapter adapter : mAdapters) {
            if (adapter.mViewTypesMap.containsKey(viewType)) {
                return adapter.mAdapter.onCreateViewHolder(viewGroup,
                        adapter.mViewTypesMap.get(viewType));
            }
        }
        return null;
    }
